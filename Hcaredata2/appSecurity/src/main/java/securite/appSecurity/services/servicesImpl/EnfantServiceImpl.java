...
import securite.appSecurity.services.FileStorageService;
...
public class EnfantServiceImpl implements EnfantService {
    ...
    @Override
    @Transactional
    public Enfant creer(CollectData request, List<MultipartFile> photos) {
        ...
        photos.forEach(tof->{
            fileStorageService.storeFile(tof);

            photo = new Photo();

            photo.setImage(tof.getOriginalFilename());
      ...
      if(villeOptional.isEmpty()) throw new MyException("aucune ville de "+pays.getLibelle()+" avec l'id "+ request.getLocalisation().getVille());
       ...
       medicalData.setTaille(request.getMedicalData().getTaille());
       medicalData.setAllergie(request.getMedicalData().getAllergie());
       medicalData.setAntecedent(request.getMedicalData().getAntecedent());
       medicalData.setPoids(request.getMedicalData().getPoids());
       medicalData.setVaccins(request.getMedicalData().getVaccins());
       medicalData.setTraitements(request.getMedicalData().getTraitements());
       medicalData.setDateVulnerabilite(request.getMedicalData().getDateVulnerabilite());
